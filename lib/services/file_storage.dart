...
class FileStorage {
  ...
  static Future<void> initialise() async {
    ...
    if (Platform.isAndroid) {
      directory = Directory(Hive.box('SETTINGS').get('APP_FOLDER',defaultValue:'/storage/emulated/0/Download/Vel_MuSic'));
    } else if (Platform.isWindows) {
      directory =
          Directory(path.join((await getDownloadsDirectory())!.path, 'Vel_MuSic'));
    } else {
  ...
  Future<void> loadBackup(BuildContext context) async {
    ...
    if (backup['name'] != 'Vel_MuSic' && backup['type'] != 'backup') {
      return;
    }
